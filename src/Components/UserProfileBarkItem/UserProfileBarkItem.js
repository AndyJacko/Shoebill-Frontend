import React from "react";
import { useNavigate } from "react-router-dom";

import "./UserProfileBarkItem.css";

const UserProfileBarkItem = ({ addLikes, loggedInUser, post }) => {
  const navigate = useNavigate();

  const deletePostHandler = async () => {
    if (window.confirm("Delete this post?")) {
      const response = await fetch(
        `https://cnmaster-shoebill.herokuapp.com/deletePost/${post._id}`,
        {
          method: "DELETE",
          body: JSON.stringify({ userId: post.user._id }),
          headers: {
            "Content-Type": "application/json",
          },
        }
      );
      const data = await response.json();

      if (data.message) {
        navigate(`/users/${loggedInUser._id}`);
      }
    }
  };

  const addLikeHandler = () => {
    addLikes(post._id);
  };

  return (
    <div className="bark-item-container">
      <div className="bark-item-date">
        {new Date(post.posteddate).toLocaleString("en-GB", {
          year: "numeric",
          month: "short",
          day: "numeric",
        })}
      </div>

      <div className="bark-item-comment">{post.postcomment}</div>

      {post.postpic && (
        <div className="home-page-bark-item-pic">
          <img src={post.postpic} alt="" />
        </div>
      )}

      <div className="bark-item-crl-container">
        <div className="bark-item-crl">
          <div className="bark-item-icon">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
              <path d="M512 240c0 114.9-114.6 208-256 208c-37.1 0-72.3-6.4-104.1-17.9c-11.9 8.7-31.3 20.6-54.3 30.6C73.6 471.1 44.7 480 16 480c-6.5 0-12.3-3.9-14.8-9.9c-2.5-6-1.1-12.8 3.4-17.4l0 0 0 0 0 0 0 0 .3-.3c.3-.3 .7-.7 1.3-1.4c1.1-1.2 2.8-3.1 4.9-5.7c4.1-5 9.6-12.4 15.2-21.6c10-16.6 19.5-38.4 21.4-62.9C17.7 326.8 0 285.1 0 240C0 125.1 114.6 32 256 32s256 93.1 256 208z" />
            </svg>
          </div>
          <div>{post.numcomments}</div>
        </div>

        <div className="bark-item-crl">
          <div className="bark-item-icon">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
              <path d="M272 416c17.7 0 32-14.3 32-32s-14.3-32-32-32H160c-17.7 0-32-14.3-32-32V192h32c12.9 0 24.6-7.8 29.6-19.8s2.2-25.7-6.9-34.9l-64-64c-12.5-12.5-32.8-12.5-45.3 0l-64 64c-9.2 9.2-11.9 22.9-6.9 34.9s16.6 19.8 29.6 19.8l32 0 0 128c0 53 43 96 96 96H272zM304 96c-17.7 0-32 14.3-32 32s14.3 32 32 32l112 0c17.7 0 32 14.3 32 32l0 128H416c-12.9 0-24.6 7.8-29.6 19.8s-2.2 25.7 6.9 34.9l64 64c12.5 12.5 32.8 12.5 45.3 0l64-64c9.2-9.2 11.9-22.9 6.9-34.9s-16.6-19.8-29.6-19.8l-32 0V192c0-53-43-96-96-96L304 96z" />
            </svg>
          </div>
          <div>{post.rebarks}</div>
        </div>

        <div className="bark-item-crl add-like" onClick={addLikeHandler}>
          <div className="bark-item-icon">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
              <path d="M47.6 300.4L228.3 469.1c7.5 7 17.4 10.9 27.7 10.9s20.2-3.9 27.7-10.9L464.4 300.4c30.4-28.3 47.6-68 47.6-109.5v-5.8c0-69.9-50.5-129.5-119.4-141C347 36.5 300.6 51.4 268 84L256 96 244 84c-32.6-32.6-79-47.5-124.6-39.9C50.5 55.6 0 115.2 0 185.1v5.8c0 41.5 17.2 81.2 47.6 109.5z" />
            </svg>
          </div>
          <div>{post.likes}</div>
        </div>

        {loggedInUser && loggedInUser._id === post.user && (
          <div className="home-page-bark-item-crl">
            <div className="delete-post-icon" onClick={deletePostHandler}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                <path d="M135.2 17.7C140.6 6.8 151.7 0 163.8 0H284.2c12.1 0 23.2 6.8 28.6 17.7L320 32h96c17.7 0 32 14.3 32 32s-14.3 32-32 32H32C14.3 96 0 81.7 0 64S14.3 32 32 32h96l7.2-14.3zM32 128H416V448c0 35.3-28.7 64-64 64H96c-35.3 0-64-28.7-64-64V128zm96 64c-8.8 0-16 7.2-16 16V432c0 8.8 7.2 16 16 16s16-7.2 16-16V208c0-8.8-7.2-16-16-16zm96 0c-8.8 0-16 7.2-16 16V432c0 8.8 7.2 16 16 16s16-7.2 16-16V208c0-8.8-7.2-16-16-16zm96 0c-8.8 0-16 7.2-16 16V432c0 8.8 7.2 16 16 16s16-7.2 16-16V208c0-8.8-7.2-16-16-16z" />
              </svg>
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

export default UserProfileBarkItem;
